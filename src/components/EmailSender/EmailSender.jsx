...
const EmailSender = ({
  ...
  useEffect(() => {
    const result = contacts?.filter(
      (item) => !emails?.some((el) => el.email === item.e_mail)
    );
  ...
  const handleSendEmail = () => {
    const dataForSend = {
      emails:
        sendCopy && partnerEmail && partnerEmail !== ""
          ? [
              ...emails.map((el) => {
                return el.email;
              }),
    ...
    sendUpd({ body: dataForSend, id }).then((res) => {
      if (res.data.success) {
        setOpen(false);
        handleSendEmailSuccess();
  ...
  const handleAddEmail = (el) => {
    ...
    uniq &&
      setEmails((prevState) => [
        ...prevState,
        { email: el?.e_mail, name: `${el.name} ${el.surname}` },
      ]);
  };
