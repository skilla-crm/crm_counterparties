...
//api
import { useSendDebtNotificationMutation } from '../../redux/services/emailSenderApiActions';
//icons
...
const EmailSender = ({
  ...
  const [sendDebtNotification, { data, isError, isLoading }] = useSendDebtNotificationMutation();
  ...
  useEffect(() => {
    const result = contacts?.filter((item) => !emails?.some((el) => el.e_mail === item.e_mail) && item.e_mail.trim() !== '');
  ...
  const handleSendEmail = async () => {
    const { format, sign } = formatMap[formatDoc] || { format: 'docx', sign: 0 };
    ...
    try {
      const res = await sendDebtNotification(dataForSend).unwrap();
      if (res.success) {
        handleSendEmailSuccess?.();
  ...
  const handleWriteEmail = () => {
    ...
    if (email.length > 0) {
      if (valid && uniq) {
        ...
        setEmails((prevState) => [
          ...prevState,
          { email: email, name: existingEmail ? existingEmail?.name : '' },
        ]);
        setEmailValue('');
        ...
        uniq ? setErrorText('Неверный эл.адрес') : setErrorText('Email уже выбран для отправки');
      }
      return;
    }
  };
    ...
    <div className={classNames(s.window, open && s.window_open)}>
      <div ref={modalRef} className={classNames(s.modal, open && s.modal_open)}>
        ...
        <div className={s.container}>
          <div className={s.block}>
            ...
            <div ref={emailsRef} onClick={handleFocusInput} className={s.emails}>
              {emails?.map((el, i) => {
                return <Email key={el.email} el={el} emails={emails} setEmails={setEmails} />;
              })}
              <div
                ...
                ref={textAreaRef}
                rows={1}
                className={classNames(s.emailArea, emailError && s.emailArea_error)}
              ></div>
            </div>
            <div ref={contactsRef} className={s.container_contacts}>
              <ul className={classNames(s.contacts, openContacts && s.contacts_open)}>
            ...
            <span className={classNames(s.text, emailError && s.text_red)}>{errorText}</span>
          </div>
          ...
          {/* <div className={s.block}>
            ...
            <div className={s.switches}>
              <Switch text={'Акт сверки'} switchState={true} disabled={true} />
          ...
          <div className={s.block}>
            <span>Формат</span>
            <FormatList formats={formats} format={formatDoc} setFormat={setFormatDoc} />
          </div>
        ...
        <div className={s.buttons}>
          <button onClick={handleClose} disabled={isLoading} className={s.cancel}>
            Отмена
            <IconCloseBlue />
          </button>
...
const Email = ({ el, emails, setEmails }) => {
 ...
 /*    setHidden(true); */
  ...
  return (
    <div id={el.e_mail} className={classNames(s.item, anim && s.item_anim, hidden && s.item_hidden)}>
      <span>{el.e_mail}</span>
      {el.name && <p>{el.name}</p>}
