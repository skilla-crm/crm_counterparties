...
const InputData2 = ({ sub, nosub, setDate, date }) => {
    ...
    const handleDateText = () => {
        ...
        const today =
            dayjs(date).format('D MM YYYY') == dayjs().format('D MM YYYY');
        const tomorrow =
            dayjs(date).format('MM YYYY') == dayjs().format('MM YYYY') &&
            dayjs(date).format('D') - dayjs().format('D') == 1;
        const dayAfterTomorrow =
            dayjs(date).format('MM YYYY') == dayjs().format('MM YYYY') &&
            dayjs(date).format('D') - dayjs().format('D') == 2;
    ...
    const handleDateValue = (e) => {
        ...
        value.length == 2 &&
            dateVis.length < 2 &&
            setDateVis((prevState) => prevState + '.');
        value.length == 5 &&
            dateVis.length < 5 &&
            setDateVis((prevState) => prevState + '.');
        const isValid = dayjs(value, 'DD.MM.YYYY', true).isValid();
        isValid
            ? setDate(dayjs(value, 'DD.MM.YYYY').locale('ru'))
            : setDate(null);
        value == '' && setDate(null);
    };
    ...
    return (
        <div className={`${s.container}`}>
            ...
            <div
                ...
                className={`${s.field} ${s.field_data} ${fieldFocus && s.field_focus}`}
            >
                <input
                    ref={inputRef}
                    onFocus={handleFocus}
            ...
            {!nosub && (
                <div className={s.bages}>
                    <p onClick={handleBage} id="1">
                        Завтра
                    </p>
                    <p onClick={handleBage} id="2">
                        Послезавтра
                    </p>
                </div>
            )}
