...
//slice
import { setCustomer, setDetail, setNumberBill, setDate, setSignatory, setContract, setDateContract, setNumContract, setNds } from '../../redux/mainInfo/slice';
...
const MainInfoBlock = ({ parameters, disabled }) => {
    const role = document.getElementById('root_reconciliation')?.getAttribute('role');
    const dispatch = useDispatch()
    const { customer, detail, numberBill, numberBillFirst, date, orders, signatory, draft, contract, numContract, dateContract, nds } = useSelector((state) => state.mainInfo);
    ...
    const handleOpenOrder = (e) => {
        ...
        role === 'director' && window.open(`https://lk.skilla.ru/new/orders/order_detail/${orderId}`, '_blank')
    }
        ...
        <div className={s.root}>
            ...
            <DropDown
                ...
                setActiveItem={data => {
                    ...
                    dispatch(setDateContract(dayjs(data?.contract_date).format('YYYY-MM-DD')))
                    dispatch(setSignatory(data?.gendir && data?.gendir?.replace(/\s+/g, '') !== ''
                        ? { id: 'dir', name: data?.gendir } : { id: 'no', name: 'Без подписанта' }))
                }}
            ...
            {orders.length > 0 && <div className={classNames(s.orders, role === 'accountant' && s.orders_dis)}>
