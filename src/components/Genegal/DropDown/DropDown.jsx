...
const DropDown = ({
  ...
  useEffect(() => {
    if (activeItem?.id || activeItem?.partnership_id) {
      ...
      type == "customer" &&
        setCaption(
          `${activeItem?.inn ? `ИНН ${activeItem?.inn}` : ""} ${
            activeItem?.kpp ? `КПП ${activeItem?.kpp}` : ""
          }`
        );
      ...
      type == "detail" &&
        setCaption(
          `${activeItem?.bank} ${
            activeItem?.rs?.length > 4
              ? `*${activeItem?.rs?.slice(-4)}`
              : activeItem?.rs
          }`
        );

      type == "position" &&
        activeItem?.name_service &&
        activeItem?.name_service !== "" &&
        setQuery(activeItem?.name_service);
      type == "position" &&
        (!activeItem?.name_service || activeItem?.name_service == "") &&
        setQuery(activeItem?.name_button);

      type == "signatory" &&
        activeItem?.surname &&
        setQuery(`${activeItem?.name} ${activeItem?.surname}`);
      type == "signatory" &&
        !activeItem?.surname &&
        setQuery(`${activeItem?.name}`);
      type == "signatory" &&
        activeItem?.e_mail &&
        setCaption(activeItem?.e_mail);
  ...
  return (
    <div
      className={classNames(
        s.root,
        type == "signatory" && s.root_wide,
        noActive && s.root_noactive
      )}
    >
      {sub && sub !== "" && <span className={s.sub}>{sub}</span>}
      <div
        ref={fieldRef}
        onClick={overlay ? handleFocus : null}
        className={classNames(
          s.field,
          disabled && s.field_disabled,
          disabled && !overlay && s.field_disabledover,
          error && s.field_error
        )}
      >
        <input
        ...
        {overlay && (
          <div
            className={classNames(
              s.overlay,
              !openList && s.overlay_active,
              disabled && s.overlay_disabled
            )}
          >
            <p>{query}</p>
            {caption.length > 0 && !openList && <span>{caption}</span>}
            {lable.length > 0 && !openList && (
        ...
        <IconChewron
          onClick={handleChoseActiveItem}
          className={classNames(s.chewron, openList && s.chewron_open)}
        />
      </div>
      <ul
        ref={modalRef}
        ...
        className={classNames(
          s.block,
          openList && s.block_open,
          searchList?.length > 6 && s.block_scroll,
          type == "position" && s.block_position
        )}
      >
        ...
        {!noActive && (
          <div /* ref={parent}  */ className={s.list}>
            {searchList?.length === 0 && (
              <div className={s.notfound}>Не найдено по запросу “{query}”</div>
            )}
            {searchList?.map((el, i) => {
              return (
                <div
                  key={el.id ? el.id : i}
                  onClick={() => handleChoseActiveItem(el)}
                >
                  <ListItem el={el} />
                </div>
              );
      ...
      <span className={classNames(s.error, error && s.error_vis)}>
        <IconWarning /> {errorText}
      </span>
    </div>
  );
};
