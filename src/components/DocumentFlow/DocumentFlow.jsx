...
const BlockSend = ({ handleClose, id }) => {
  const [sendOriginal, { data, isError, isLoading }] =
    useSendOriginalUpdMutation();
  ...
  const handleSendOriginal = () => {
    ...
    sendOriginal({ body: dataForSend, id }).then((res) => {
...
const BlockSign = ({ handleClose, id }) => {
  const [signOriginal, { data, isError, isLoading }] =
    useSignOriginalUpdMutation();
  const [date, setDate] = useState(dayjs().locale("ru") || null);
  ...
  const handleSignOriginal = () => {
    ...
    signOriginal({ body: dataForSend, id }).then((res) => {
      res.data.success && handleClose();
    });
  };
...
const BlockReject = ({ handleClose, type, id }) => {
  ...
  const handleUnmark = (e) => {
    ...
    unmarkUpd({ body: { type: idType }, id }).then((res) => {
      res.data.success && handleClose();
    });
  };
  return (
