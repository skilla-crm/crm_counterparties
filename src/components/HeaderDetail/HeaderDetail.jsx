...
const HeaderDetail = ({ id, type, setType }) => {
    const { customer, detail, signatory, contract, dateContract, numContract, numberBill, date, nds } = useSelector((state) => state.mainInfo);
    ...
    const handleValidation = () => {
        ...
        const positionsValidation = positions.every(el => el?.rate?.name_service !== '' && el?.rate?.name_service && Number(el?.count) > 0 && el?.count !== '' && el?.units !== '' && Number(el?.code) > 0 && Number(el?.price) >= 0 && Number(el?.total) >= 0);
