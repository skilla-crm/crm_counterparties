...
const TableHeader = ({ type }) => {
  if (type === "approved") {
    return (
      <div className={classNames(s.gridHeader, s.approved)} key={"approved"}>
        ...
        <div className={classNames(s.gridCell, s.right)}>Статус ЕГРЮЛ </div>
        <div className={classNames(s.gridCell, s.center)}>Примечание</div>
      </div>
    );
  }
