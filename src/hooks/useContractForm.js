...
export const useContractForm = () => {
    const [form, setForm] = useState({
        ...
        label: '',
        individual_pricelist: 0, // индивидуальный прайс-лист
    });
    ...
    const getFormData = useCallback(
        (priceRates = []) => {
            ...
            // Прайс-лист
            if (cleaned.individual_pricelist && priceRates.length > 0) {
                const priceData = handlePreparePriceData(priceRates);
                // Копируем все поля из priceData в formData
