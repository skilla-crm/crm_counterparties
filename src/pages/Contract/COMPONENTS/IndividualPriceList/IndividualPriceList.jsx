...
const IndividualPriceList = ({ data = [] }) => {
    ...
    return (
        <div className={s.root}>
            <div className={s.header}>
                ...
                <button
                    className={s.openListBtn}
                    onClick={handleOpenList}
                >
                    <IconArrowDown
                        className={classNames(
                            s.arrow,
                            openList && s.arrow_open
                        )}
                    />
                </button>
            </div>
            {priceRates.length > 0 && openList && (
                <span className={s.infoTitle}>
                    Занесенный 0 в графу означает - "по согласованию". Идет
                    только в договор. Ставка рабочим не публикуется.
                </span>
            )}
