...
// Components
import InputAddress from "components/General/InputAddress/InputAddress";
import InputData2 from "components/General/InputData/InputData2";
...
import { ReactComponent as IconDocSuccess } from "assets/icons/iconDocSuccessBlue.svg";
  ...
  return (
    <div ref={modalRef} className={s.root}>
      <div className={s.block}>
        ...
        <div
          onClick={handleContainerStatus}
          id={exchange?.send === 1 ? 3 : 1}
          className={s.item}
        >
          {exchange?.send === 1 ? <IconDoneGreen /> : <IconMailBlue />}
          {exchange?.send !== 1 && <p>Подтвердить отправку оригиналов</p>}
          {exchange?.send === 1 && exchange?.send_type === "self_pickup" && (
            <p>Бумажный оригинал передан</p>
          )}
          {exchange?.send === 1 && exchange?.send_type === "courier" && (
            <p>Бумажный оригинал отправлен</p>
          )}
          {exchange?.send === 1 && exchange?.send_type === "edo" && (
            <p>Оригинал отправлен по ЭДО</p>
          )}
          {exchange?.send === 1 && exchange?.send_type === "post" && (
            <p>Бумажный оригинал отправлен</p>
          )}
        </div>
        ...
        <div
          onClick={handleContainerStatus}
          id={exchange?.sign === 1 ? 4 : 2}
          className={classNames(s.item, exchange?.send !== 1 && s.item_hidden)}
        >
          {exchange?.sign === 1 ? <IconDoneGreen /> : <IconDocSuccess />}
          {exchange?.sign !== 1 && <p>Подтвердить подписание</p>}
          {exchange?.sign === 1 && exchange?.sign_type === "edo" && (
            <p>Подписан по ЭДО</p>
          )}
          {exchange?.sign === 1 && exchange?.sign_type === "pepper" && (
            <p>Подписан бумажный оригинал</p>
          )}
        </div>
      </div>
      ...
      <div
        className={classNames(
          s.container,
          containerStatus === 1 && s.container_1,
          containerStatus === 2 && s.container_2,
          (containerStatus === 3 || containerStatus === 4) && s.container_3
        )}
      >
        {blockStatus === 1 && <BlockSend handleClose={handleClose} id={id} />}
        {blockStatus === 2 && <BlockSign handleClose={handleClose} id={id} />}
        {blockStatus === 3 && (
          <BlockReject handleClose={handleClose} type={"send"} id={id} />
        )}
        {blockStatus === 4 && (
          <BlockReject handleClose={handleClose} type={"sign"} id={id} />
        )}
      </div>
    </div>
  );
...
const BlockSend = ({ handleClose, id }) => {
  const [sendOriginalContract, { data, isError, isLoading }] =
    useSendOriginalContractMutation();
  ...
  const handleSendOriginal = () => {
    ...
    sendOriginalContract({ data: dataForSend, id })
      .unwrap()
      .then((res) => {
        res.success && handleClose();
      })
      .catch((err) => console.log(err));
  };

  const list = [
    { id: "self_pickup", name: "Самовывоз" },
    { id: "courier", name: "Курьером" },
    { id: "edo", name: "Отправлен по ЭДО" },
    { id: "post", name: "Почтой" },
  ];
  return (
    <div className={classNames(s.block, s.block_inner)}>
      <div className={s.header}>
        <h3>
          <IconMailBlack /> Отметка об отправке документа
        </h3>
      ...
      <RadioButtons
        list={list}
        active={sendType}
        setActive={setSendType}
        sub={"Способ отправки"}
      />
      <div
        className={classNames(s.address, sendType == "post" && s.address_open)}
      >
...
const BlockSign = ({ handleClose, id }) => {
  const [signOriginalContract, { data, isError, isLoading }] =
    useSignOriginalContractMutation();
  const [date, setDate] = useState(dayjs().locale("ru") || null);
  ...
  const handleSignOriginal = () => {
    ...
    signOriginalContract({ data: dataForSend, id })
      .unwrap()
      .then((res) => {
        Boolean(res.success) && handleClose();
      })
      .catch((err) => console.log(err));
  };

  const list = [
    { id: "pepper", name: "Оригинал на бумаге" },
    { id: "edo", name: "ЭДО" },
  ];
  return (
    <div className={classNames(s.block, s.block_inner)}>
      <div className={s.header}>
        <h3>
          <IconDocSuccessBlack /> Отметка о подписи оригинала
        </h3>
        <IconClose onClick={handleClose} className={s.close} />
      </div>
      ...
      <RadioButtons
        ...
        active={sendType}
        setActive={setSendType}
        sub={"Способ, которым документ был подписан обеими сторонами"}
      />
...
const BlockReject = ({ handleClose, type, id }) => {
  const [unmarkOriginalContract, { data, isError, isLoading }] =
    useUnmarkOriginalContractMutation();

  const handleUnmark = (e) => {
    const idType = e.currentTarget.id;
    unmarkOriginalContract({ data: { type: idType }, id })
      .unwrap()
      .then((res) => {
        res.success && handleClose();
      })
  ...
  return (
    <div className={classNames(s.block, s.block_inner)}>
      ...
      <button
        ...
        id={type}
        className={classNames(s.button, s.button_red)}
      >
        <div className={classNames(s.loader, isLoading && s.loader_vis)}>
          <LoaderButton color={"#fff"} />
        </div>
