...
export const Contract = () => {
    ...
    useEffect(() => {
        ...
        const fields = {
            ...
            partnership_details_id:
                ...
                scopedSettings?.partnerships?.[0]?.details?.find(
                    (el) => el.is_main
                )?.id ||
                null,
            number:
                scopedSettings?.partnerships?.find((el) => el.is_main)
                    ?.contract_num ||
            ...
            contract_template_id:
                scopedSettings?.contract_templates?.[0]?.id || null,
            docs: [],
        };
    ...
    // Инициализация прайс листа договора
    useEffect(() => {
        if (!contractData) return;

        if (form.individual_pricelist === 1) {
            const priceListData = Array.isArray(
    ...
    }, [contractData, form.individual_pricelist, dispatch]);
        ...
        <div className={s.wrapper}>
            ...
            <div
                className={classNames(
                    s.root,
                    isContractLoading && s.rootLoading
                )}
            >
                <ContractHeader
                    // refetch={refetchCounterparty}
