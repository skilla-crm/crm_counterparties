...
const Detail = () => {
  ...
  useEffect(() => {
    if (data) {
      ...
      dispatch(
        setContract(data?.details?.contract ? data?.details?.contract : "")
      );
      dispatch(
        setDateContract(
          data?.details?.contract_date ? data?.details?.contract_date : ""
        )
      );
      dispatch(
        setNumContract(
          data?.details?.contract_n ? data?.details?.contract_n : ""
        )
      );
      if (data?.draft === 1) {
        ...
        dispatch(
          setDateContract(
            data?.company?.contract_date ? data?.company?.contract_date : ""
          )
        );
        dispatch(
          setNumContract(
            data?.company?.contract_n ? data?.company?.contract_n : ""
          )
        );
      }
      ...
      if (data?.details?.company_contact_id) {
        dispatch(
          setSignatory({
            id: data?.details?.company_contact_id,
            name: data?.details?.signature,
          })
        );
      } else if (data?.details?.signature) {
        dispatch(
          setSignatory({ id: "another", name: data?.details?.signature })
        );
      } else {
