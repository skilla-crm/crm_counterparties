    ...
    <div className={s.root}>
      {data.length > 0 && (
        <div className={s.infoTitle}>
          <span>
            Активные представители будут по умолчанию выбраны для рассылок
      ...
      {data.length > 0 ? (
        ...
        <div className={s.empty}>Пока не добавлен ни один контакт</div>
      )}
    </div>
  );
};
...
const ContactRow = ({ contact, counterpartyId }) => {
  ...
  const handleSwitchStatus = (e) => {
    ...
    switchContactStatus({
      ...
      .then((res) => {
        if (
          res.message === "Company contact active status changed successfully"
        ) {
      ...
      <div>
        {contact.phone && (
          <div className={s.flexElem}>
            ...
            {Boolean(contact.dob) && (
              <span className={s.dob}>{`(${contact.dob})`}</span>
            )}
          </div>
        )}
      </div>
